import { and, asc, count, desc, eq, getTableName, inArray, isNull, sql } from "drizzle-orm";
import { db } from "../../db/dbConnection";
import { DBNewRecord, DBNewRecords, DBTable, DBTableRow, InQueryData, OrderByQueryData, PaginationInfo, UpdateRecordData, WhereQueryData } from "../../types/db.types";
...
/*
const countRecordsWithFilters = async <R>(
  table: DBTable,
  filters?: { column: string; value: any }[]
): Promise<number> => {

  console.log("countRecordsWithFilters", filters);

  let initialQuery = db.select(and(eq{ total: count() table.status,vaue }).from(table);
  ...
  if (filters && filters.length > 0) {
    ...
    initialQuery = initialQuery.where(and(...filterConditions)) as typeof initialQuery;
  }

  const result = await initialQuery;


  // Return the total count or 0 if no result is found
  return result[0]?.total || 0;
}; */
...
export {
  deleteRecordById, exportData, getMultipleRecordsByAColumnValue,
  ...
  //getRecordByColumn,
  getRecordsCount, getSingleRecordByAColumnValue,
  getSingleRecordByMultipleColumnValues, saveRecords, saveSingleRecord, softDeleteRecordById,
  updateMultipleRecords, updateRecordById, deleteRecordByCondition, getRecordsCountByCondition
};
